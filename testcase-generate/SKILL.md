...
**Step 1：盘点输入文件 -> Step 2：先分析原始需求并让用户确认 -> Step 3：补齐相关 Drive 文档 -> Step 4：查询并摘要 Jira 并让用户确认 -> Step 5：整合资料、补齐历史矩阵并统一确认测试设计 -> Step 6：生成并 review 测试用例**
...
4. 是否遗漏需求中的差异化分支，例如：
   ...
   - 默认允许 vs 默认拒绝
5. 是否存在“看起来覆盖了，其实粒度过粗”的情况；若一条 case 混了多个核心结果，必须拆开。
6. 是否存在历史 Jira / 设计文档已经暴露的高风险场景，但终稿没有体现。
7. 是否存在越界内容：与当前 feature 仅弱相关、只能算背景信息的内容，不应大面积进入最终用例。
8. 是否与 `references/testrail_default.md` 的结构模板对齐：
...
- 先在内部完成 review，再直接修改 `testcase_a.md`、`testcase_b.md`、`testcase_final.md`，不要把未修订的草案直接交给用户。
