...
进入 Step 4 前，先根据 Step 2 已确认的 mode 初始化统一工作目录。默认复用 Step 0 已创建的 intake working_dir：
...
- 不要删除 Step 0 留下的 `input-manifest.json`；它是这轮输入确认的记录。
...
- 如果 Step 3 用户明确表示不补充 Drive 文档，不要因为缺少 `drive_doc` 就阻塞 Step 4；直接基于当前已上传 / 已确认的资料继续。
