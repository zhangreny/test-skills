...
根据给定的 Google Drive 文件 ID 与目标目录，逐个调用 `scripts/drive_download_by_id.py` 脚本将文件下载到本地下的 `google-drive-file-download` 文件夹。每个 ID 单独执行，便于错误定位与重试；失败时向用户反馈具体错误信息。

## 前置条件

- 本技能依赖脚本：`scripts/drive_download_by_id.py`。执行前确认脚本存在。
...
| 目标目录不存在 | 报错并停止，不执行下载 |
| 脚本不存在 | 提示检查技能目录下 `scripts/drive_download_by_id.py` 是否存在 |
