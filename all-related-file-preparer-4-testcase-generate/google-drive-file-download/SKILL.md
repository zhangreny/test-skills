...
根据给定的 Google Drive 文件 ID 与目标目录，逐个调用 scripts/drive_download_by_ids.py 脚本将文件下载到本地下的 google-drive-file-download 文件夹。每个 ID 单独执行，便于错误定位与重试；失败时向用户反馈具体错误信息。

## 前置条件

- 本技能依赖脚本：`scripts/drive_download_by_ids.py`。执行前确认脚本存在。
...
```bash
mkdir -p <目标目录>/google-drive-file-download
```

**必须**在循环中**依次只传递和下载一个 ID**：对列表中的每个文件 ID，**单独调用一次**脚本，每次调用只传入一个 ID，实现「一次只下载一个文件」。不得在一次调用中传入多个 ID。

```bash
# SKILL_DIR 为本技能的安装目录，需替换为实际路径
# 对每个文件 ID 分别执行一次，每次只传一个 id
python3 "${SKILL_DIR}/scripts/drive_download_by_id.py" --output "<目标目录>/google-drive-file-download" --id "<单个文件ID>"
```
...
- 若脚本退出码非 0，表示至少有一个下载失败；需将 **stderr 中的具体错误** 整理后反馈给用户，便于用户针对失败 ID 重试或排查（权限、网络、id 无效等）。
...
| 目标目录不存在 | 报错并停止，不执行下载 |
| 脚本不存在 | 提示检查技能目录下 `scripts/drive_download_by_ids.py` 是否存在 |
| 脚本返回非零 | 捕获 stderr，将每个失败 ID 及对应错误反馈给用户，建议重试 |
