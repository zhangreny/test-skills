...
After every markdown download, always run the cleanup script below to strip
embedded base64 image reference blocks such as
`[image1]: <data:image/png;base64,...>`.

Script path:

```text
<SKILL_DIR>/scripts/strip_markdown_base64_images.py
```

Required command after a successful markdown download:

```text
python <SKILL_DIR>/scripts/strip_markdown_base64_images.py "<target_dir>/google-drive-file-download"
```

This applies to both paths:

- the normal `scripts/drive_download_by_id.py` path
- the fallback `gog drive download ... --format md` path
...
After the fallback command succeeds, immediately run:

```text
python <SKILL_DIR>/scripts/strip_markdown_base64_images.py "<目标目录>/google-drive-file-download"
```
