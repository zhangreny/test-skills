---
title: "SMTXZBS/5.7.0/SMTX ZBS 报警系统对接 Zabbix"
source_url: "https://internal-docs.smartx.com/smtxzbs/5.7.0/zabbix_alert/zabbix_alert_preface"
sections: 17
---

# SMTXZBS/5.7.0/SMTX ZBS 报警系统对接 Zabbix
## 关于本文档

# 关于本文档

SMTX ZBS 内置有报警功能，并提供了图形化界面对集群报警进行管理。但在实际场景中，部分客户也会使用第三方工具管理集群的报警信息，典型的第三方工具为 Zabbix。

本文档介绍了如何在 Zabbix 端和 SMTX ZBS 端进行配置，将 SMTX ZBS 的报警系统与 Zabbix 进行对接，从而实现通过 Zabbix 监控集群的报警信息。
...
snmptrapd 用于 Zabbix 接收 SMTX ZBS 集群发出的 SNMP Trap 信息。请执行以下命令安装并启用 snmptrapd 服务。
...
当 Zabbix 对接 SMTX ZBS 报警系统时，需创建 SNMP Host，并为 Host 创建一个 SNMP Item，用以监控集群的报警信息。其中，SNMP Host 定义了 Zabbix 监控的 SMTX ZBS 集群，SNMP Item 为监控的数据类型。

---

## 配置 Zabbix > 创建 SNMP Host

# 创建 SNMP Host

一个 SMTX ZBS 集群可以创建多个 SNMP Host，若需监控多个集群，请为每个集群和每个可观测性服务分别创建 SNMP Host。
...
2. 进入 **Configuration** > **Hosts** 界面 ，单击 **Create host**，弹出如下配置界面，并配置相关参数。
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_14.png)

   | 参数 | 描述 |
   | --- | --- |
   | Host name | 输入 SNMP Host 的名称。 |
   | Groups | 选择 Host 所在的 Group。 |
   | SNMP interfaces | 根据以下规则输入 IP 地址，并将端口设置为 `161`： - 若集群已关联可观测性服务，则输入可观测性服务的 IP 地址。 - 若集群未关联可观测性服务，则：   - 若集群已配置 VIP，则输入集群的 VIP 地址。   - 若集群未配置 VIP，则输入 SMTX ZBS 集群中 zbs-taskd leader 节点的管理 IP。若 Zabbix 与 SMTX ZBS 节点跨网关，请同时添加网关 IP 地址。 |
3. 单击 **Add**，完成 SNMP Host 的创建。
4. 若该集群未配置 VIP，则重复步骤 2-3，为集群内的每个 master 节点创建 SNMP Host。在配置 SNMP interfaces 参数时，输入当前节点的管理 IP 地址，若 Zabbix 与 SMTX ZBS 节点跨网关，请同时添加网关 IP 地址。
5. （可选）若客户直接使用 Zabbix Server 接受报警信息则可跳过此步骤。当客户需要通过 Zabbix Proxy 接收集群发送的报警信息时，则可在 **Host** 页面的 **Monitored by proxy** 下拉框选择对应的 Zabbix Proxy 完成配置，如下图所示。
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_15.png)

---

## 配置 Zabbix > 创建 SNMP Item
...
2. 在 **Item** 配置界面，配置如下参数。
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_16.png)
...
4. （可选）解析中文报警文案。
   ...
   当客户环境已安装 snmptt 时，您可通过如下步骤增加中文字符处理。配置后，报警文案将自动被解析为中文字符进行展示。若未安装 snmptt，请参考[中文报警对接](/smtxzbs/5.7.0/zabbix_alert/zabbix_alert_12#%E4%B8%AD%E6%96%87%E6%8A%A5%E8%AD%A6%E5%AF%B9%E6%8E%A5)中的步骤进行操作以进行中文解析。
   ...
   1. 在 **Create item** 界面，单击 **PreProcessing**，进入配置界面。
      ...
      ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_17.png)
   ...
   3. 单击界面右侧的 **Test** 按钮，并在弹出的对话框中输入如下信息，以测试配置是否生效。
      ...
      - **中文解析结果**：
        ...
        ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_18.png)
      - **英文解析结果**：

        ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_19.png)

---

## 配置 Zabbix > 格式化报警内容（未安装 snmptt）
...
1. 在 **Create item** 界面，单击 **PreProcessing**，进入格式化界面。
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_20.png)
2. 添加正则表达式提取报警内容。
   ...
   **正则表达式**：
   ...
   - 第 3 组：`(\d+\.\d+\.\d+\.\d+)` 用于匹配 SMTX ZBS 集群的 IP 地址；
   ...
   **匹配结果**：
   ...
   - `ip(\3)` 表示使用第 3 组正则表达式过滤出 SMTX ZBS 集群的 IP 地址；
...
1. 添加正则表达式。
   ...
   **匹配结果**：
   ...
   - 第 1 组 `\”(.*)\”` 报警级别 CRITICAL；
   - 第 2 组 `([\s\S]*?)` 用于匹配包括换行在内的任意字符；
   - 第 3 组 `(\d+\.\d+\.\d+\.\d+)` 用于匹配 SMTX ZBS 集群的 IP 地址 10.111.130.81；
...
2. 配置中文转义。
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_21.png)
...
完成 Zabbix 端和 SMTX ZBS 端的配置后，请参考以下方法验证 Zabbix 和 SMTX ZBS 的报警系统是否对接成功。

**操作步骤**

1. 进入 CloudTower 或者 SMTX ZBS 的 Web 控制台，手动解决一个报警。
...
2. 进入 Zabbix Web 前端，在 **Monitoring** > **Latest data** 页面，选择对应的 Host，点击 **History** 查看是否出现了对应的报警条目。若出现相应的报警条目，则说明配置成功。
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_22.png)

   ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_23.png)
...
Q：若 Zabbix 端无法查看 SMTX ZBS 端的报警信息，应该怎么办？

A：请参考如下思路进行问题定位：

1. 请参考以上配置介绍，确认 SMTX ZBS 端和 Zabbix Server 端已配置正确。
2. 在 SMTX ZBS 节点抓包，查看报警信息的发送是否正常。

   在 SMTX ZBS 节点，执行 `tcpdump -i port -mgt host ip -w 100.cap` 命令，抓取节点向 Zabbix Server 发送的报文，其中 `ip` 为 Zabbix Server 端的 IP 地址。查看 SMTX ZBS 节点是否向 Zabbix Server 发送了 Trap 信息。如果发送了信息，输出内容应如下图所示，若未抓取到 Trap 信息，请联系研发处理。

   ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_24.png)
3. 若 SMTX ZBS 节点能够正常发送 Trap 信息，请在 Zabbix 端进行抓包，并过滤出节点发出的报文。若 Zabbix 端未抓取到 SMTX ZBS 集群发出的报文，请检查 Zabbix 和集群之间的网络连接是否正常。
4. 若 Zabbix 可以抓取 SMTX ZBS 节点发出的 Trap 信息，请查看报文的 community 字段的值和 SNMP Host 配置的 community 值是否相同，若不相同，请重新设置 SNMP Host 的 community 参数，确保和 SMTX ZBS 中设置的 community 值相同。
5. 若 Zabbix 端能够正常收取 SMTX ZBS 节点发出的 Trap 报文，请执行如下操作。
   ...
   2. 执行 `tail -f /tmp/zabbix_traps.tmp` 命令，查看 SNMPTrapperFile 文件信息。其中 `/tmp/zabbix_traps.tmp` 为 SNMPTrapperFile 文件的路径，请以客户环境实际的路径代替。
      ...
      ![](https://cdn.smartx.com/internal-docs/assets/3f778d33/zabbix_25.png)
   3. 若在 Zabbix\_traps.tmp 可以查看到 trap 信息，说明 SMTX ZBS 集群已经将 trap 信息发送到了 Zabbix Server。此时可以确定 Zabbix Server 的 snmptrapper 未从 SNMPTrapperFile 获取数据。请执行命令 `systemctl restart snmptrapd` 和 `systemctl restart snmptt` 命令重启 snmptrapd 和 snmptt 进程，然后重新检查 Zabbix Web 前端是否可以正常查看报警信息。
   4. 若前端仍无法查看告警信息，请检查是否由于 SMTX ZBS 节点和 Zabbix 跨网络，导致 SNMPTrapperFile 文件中告警信息的 IP 地址与 SNMP Host 中设置的节点 IP 地址不同。如果两个 IP 地址不同，请手动修改将 Host 中 SNMP interfaces IP 地址修改为SNMPTrapperFile 中的 IP 地址，然后再次检查是否可以查看告警信息。
