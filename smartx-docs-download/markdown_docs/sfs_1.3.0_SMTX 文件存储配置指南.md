---
title: "sfs/1.3.0/SMTX 文件存储配置指南"
source_url: "https://internal-docs.smartx.com/sfs/1.3.0/sfs_configuration/sfs_configuration_preface"
sections: 40
---

# sfs/1.3.0/SMTX 文件存储配置指南
## 关于本文档

# 关于本文档

本文档介绍了如何通过 NFS 协议和 HDFS 协议访问 SFS 提供的文件存储。

阅读本文档需了解文件存储、NFS 协议、HDFS 协议等相关技术背景知识，并具备丰富的数据中心操作经验。
...
SMTX 文件存储（SMTX File Storage，简称“SFS”）是由北京志凌海纳科技股份有限公司（以下简称 “SmartX”）自主研发的分布式文件存储产品，可以为用户提供基于 NFS 协议和 HDFS 协议访问的文件存储服务。
...
客户端使用文件存储前，请您确认文件存储集群中已经存在状态为`已上线`的文件系统。否则，请参考《SMTX 文件存储用户指南》[创建](/sfs/1.3.0/sfs_user_guide/sfs_user_guide_20)一个新的文件系统或将已有的文件系统[上线](/sfs/1.3.0/sfs_user_guide/sfs_user_guide_26)。
...
如果 NFS 客户端运行在防火墙环境下，请参考《SMTX 文件存储防火墙端口说明》[外部防火墙端口开放规则](/sfs/1.3.0/sfs_firewall_guide/sfs_firewall_guide_01#nfs)配置 NFS 客户端的防火墙规则以允许对应端口流量出入。
...
- `<mount_path>` 替换为[挂载路径](/sfs/1.3.0/sfs_configuration/sfs_configuration_04#%E8%8E%B7%E5%8F%96%E6%8C%82%E8%BD%BD%E8%B7%AF%E5%BE%84)。若挂载路径中包含 `` ` `` ，`~` ，`!` ，`@` ，`#` ，`$` ，`%` ，`^` ，`&` ，`(` ，`)` ，`-` ，`=` ，`+` ，`_` ，`,` ，`.` ，`;` ，`[` ，`]` ，`{` ，`}` ，挂载时必须在挂载路径前后增加单引号 `'` 。若挂载路径中包含 `'` ，挂载时必须在挂载路径前后增加单引号 `'` ，并且在原本挂载命令的单引号后增加 `\''` 。
...
将文件系统挂载到 Linux 客户端的本地目录后，您可以像访问本地文件系统一样访问 SFS 的文件系统。部分版本的 Linux 限制了文件系统根目录中的部分文件操作，详情请参考 [Linux 官方文档](https://man7.org/linux/man-pages/man5/proc.5.html)中 **protected\_regular**、**protected\_hardlinks** 等描述。为了避免文件操作失败，请参考如下示例在**子目录**中操作。
...
[配置 Windows 客户端](/sfs/1.3.0/sfs_configuration/sfs_configuration_08)后，您可以通过[挂载文件系统](/sfs/1.3.0/sfs_configuration/sfs_configuration_09)到本地盘以访问 SFS，也可以[配置 UNC](/sfs/1.3.0/sfs_configuration/sfs_configuration_16) 后通过 UNC 路径访问。
...
- 请确保文件存储集群已[启用 Windows NFS 访问](/sfs/1.3.0/sfs_user_guide/sfs_user_guide_29#%E7%BC%96%E8%BE%91-windows-nfs-%E8%AE%BF%E9%97%AE)。
...
- `<mount_options>` 替换为[挂载选项](/sfs/1.3.0/sfs_configuration/sfs_configuration_09#%E6%8C%82%E8%BD%BD%E9%80%89%E9%A1%B9)，可以按需配置多项，多项间使用空格分割，且请务必配置 `nolock`。
- `<<mount_path>` 替换为[挂载路径](/sfs/1.3.0/sfs_configuration/sfs_configuration_04#%E8%8E%B7%E5%8F%96%E6%8C%82%E8%BD%BD%E8%B7%AF%E5%BE%84)。若挂载路径中包含 `'` ，`` ` `` ，`~` ，`!` ，`@` ，`#` ，`$` ，`%` ，`^` ，`&` ，`(` ，`)` ，`-` ，`=` ，`+` ，`_` ，`,` ，`.` ，`;` ，`[` ，`]` ，`{` ，`}`字符时，挂载时必须在挂载路径前后增加双引号 `"` 。
...
Universal Naming Convention (UNC) 是 Windows 系统中用于访问网络共享资源的命名约定。根据本节内容修改 Windows 客户端的 UNC 相关配置，即可通过 UNC 路径访问 SFS。

**前提条件**

**确保**[已配置区分大小写参数](/sfs/1.3.0/sfs_configuration/sfs_configuration_08#%E9%85%8D%E7%BD%AE%E5%8C%BA%E5%88%86%E5%A4%A7%E5%B0%8F%E5%86%99%E5%8F%82%E6%95%B0)，**建议**将[挂载类型设置为 hard](/sfs/1.3.0/sfs_configuration/sfs_configuration_08#%E9%85%8D%E7%BD%AE-hard-%E6%8C%82%E8%BD%BD%E7%B1%BB%E5%9E%8B)。
...
确认文件存储集群中已经存在状态为`已上线`且协议为 `HDFS` 的文件系统。否则，请参考《SMTX 文件存储用户指南》[创建](/sfs/1.3.0/sfs_user_guide/sfs_user_guide_20)一个新的文件系统或将已有的文件系统[上线](/sfs/1.3.0/sfs_user_guide/sfs_user_guide_26)。
...
- 其他特定应用如 Hadoop、Hive、HBase、Tez、StarRocks 和 Doris，请下载 SFS-hadoop 安装文件。

## 下载 Kubeconfig 文件

SFS Hadoop 客户端通过 Kubeconfig 文件访问文件存储集群的 API server、获取元数据服务位置、数据节点拓扑，请参考《SMTX 文件存储用户指南》在 CloudTower 中[下载文件存储集群 Kubeconfig 文件](/sfs/1.3.0/sfs_user_guide/sfs_user_guide_42)。
...
4. 添加 SFS 服务。
   ...
   3. 在**自定义角色分配（Assign Roles）** 页面为 SFS 服务分配主机，请选择所有需要通过 HDFS 协议访问 SFS 的主机。

      ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_01.png)
...
若您需要在 CDH 集群中添加新服务，请参考[修改新增服务配置](/sfs/1.3.0/sfs_configuration/sfs_configuration_37)完成操作。
...
- CDH 5.x
  ...
  ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_02.png)
  ...
  示例配置如下，详细配置参数及说明请参考 [SFS 参数总览](/sfs/1.3.0/sfs_configuration/sfs_configuration_27)。
...
- CDH 6.x
  ...
  1. 通过 HDFS 服务界面修改 `core-site.xml`。示例配置如 CDH 5.x，详细配置参数及说明请参考 [SFS 参数总览](/sfs/1.3.0/sfs_configuration/sfs_configuration_27)。
  2. 通过 YARN 服务界面修改 `mapreduce.application.classpath`，添加 `$HADOOP_COMMON_HOME/lib/sfs-hadoop.jar`。

     ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_03.png)
  3. 单击**更新客户端配置**，使配置生效。
  4. 通过 DistCp 将 Yarn 的相关数据迁移到 SFS。

     ```
     hadoop distcp -p  hdfs://${your_filesystem}/user/yarn  sfs://${file_system_name}/user/yarn
     ```
  5. 在 CDH 集群计算节点上执行如下命令获取 `Hadoop_version`：

     ```
     hadoop version
     ```
  6. 通过 Yarn 服务界面修改 `mapred-site.xml`：

     ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_15.png)
     ...
     <property>
       ...
       <value>sfs://${file_system_name}/user/yarn/mapreduce/mr-framework/${Hadoop_version}-mr-framework.tar.gz#mr-framework</value>
     </property>
     ```

服务配置修改后，请参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hadoop)章节内容确认 SFS 可用。

## HBase

1. 通过 HBase 服务界面修改 `hbase-site.xml`：

   ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_09.png)
...
2. 若 HBase 集群存在旧数据，为避免冲突，您可以选择删除或迁移旧数据：
   ...
   - 将原有 HBase 集群数据迁移到新的 SFS HBase 集群中。详细迁移步骤请参考[迁移数据](/sfs/1.3.0/sfs_configuration/sfs_configuration_31#hbase)章节。
3. 重启 HBase 集群，使配置生效。
...
![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_05.png)

## Spark

1. 通过 Spark 服务界面修改 `spark-defaults.conf`，添加配置项 `spark.hadoop.fs.AbstractFileSystem.sfs.impl=io.fs.SfsAbstractFileSystem`。

   ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_14.png)
2. 通过 DistCp 将 Spark 的相关数据迁移到 SFS。

   ```
   hadoop distcp -p  hdfs://${your_filesystem}/user/spark  sfs://${file_system_name}/user/spark
   ```
3. 在每个 CDH 节点上运行以下命令，为 Spark 添加 SFS Hadoop 客户端依赖：
...
通过 Cloudera Manager 修改上述服务配置后需重启 Hadoop 集群，使配置修改生效。待集群重启成功后，请参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hbase)章节内容确认 SFS 可用。
...
6. 参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hbase)章节内容确认 SFS 可用。
...
4. 恢复服务配置。
   ...
   1. 在 Cloudera Manager Web UI 中，将 CDH 中的所有[新增或修改的服务配置](/sfs/1.3.0/sfs_configuration/sfs_configuration_21)恢复原状。
...
3. 添加 SFS 服务：
   ...
   4. 在 **Customize services** 页面配置 `install_dirs`。
      ...
      ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_06.png)
   5. 按照界面指引完成剩余页面配置。若无报错，则表示安装成功。
...
2. 通过 MapReduce2 服务界面修改 `mapreduce.application.classpath`，在末尾使用 `:` 隔开现有内容，然后增加 `/usr/hdp/${hdp.version}/hadoop/lib/sfs-hadoop.jar`，无需替换变量。
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_07.png)
...
2. 若 HBase 集群存在旧数据，为避免冲突，您可以选择删除或迁移旧数据：
   ...
   - 删除原有 HBase 集群里的所有数据。通过 ZooKeeper 客户端删除 `zookeeper.znode.parent` 配置的 znode（默认 /hbase）。
   - 将原有 HBase 集群数据迁移到新的 SFS HBase 集群。详细迁移步骤请参考[迁移数据](/sfs/1.3.0/sfs_configuration/sfs_configuration_31#hbase)章节。
...
5. 通过 Tez 服务界面修改 `tez-site.xml` 中 `tez.lib.uris` 为步骤 4 的上传路径。
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_10.png)

## Hive（可选）

如需将数据库的默认创建位置设置在 SFS 上，请通过 Hive 服务界面修改 `hive.metastore.warehouse.dir` 为 SFS 上的路径。修改后，Hive 数据库和表均默认创建在 SFS 上。

![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_08.png)

## 重启服务并验证

1. 重启服务，使配置修改生效。若服务已启用 HA，滚动重启即可。
2. 参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hbase)章节内容确认 SFS 可用。
...
3. 卸载现有 SFS 服务。
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_11.png)
4. 添加新 SFS 服务。
...
7. 参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hbase)章节内容确认 SFS 可用。
...
3. 恢复服务配置。
   ...
   1. 在 Ambari Web UI 中，将 HDP 中的所有[新增或修改的服务配置](/sfs/1.3.0/sfs_configuration/sfs_configuration_25)恢复原状。
   2. 重启 Hadoop 服务，使配置修改生效。
4. 重启 Ambari Server：
...
1. 修改 `core-site.xml` 添加 SFS 参数，详细配置参数及说明请参考 [SFS 参数总览](/sfs/1.3.0/sfs_configuration/sfs_configuration_27)。
   ...
   ```
2. 参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hadoop)章节内容确认 SFS 可用。
...
2. 删除 Yarn 中的 usercache，使 Yarn 应用最新的安装文件：
   ...
   sudo rm -rf ​/yarn/nm/usercache
   ```
3. 参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hadoop)章节内容确认 SFS 可用。
...
参考 [HDFS](/sfs/1.3.0/sfs_configuration/sfs_configuration_28) 章节为 HDFS 安装 SFS Hadoop 客户端和 Kubeconfig 并修改服务配置。
...
3. 若 HBase 集群存在旧数据，为避免冲突，您可以选择删除或迁移旧数据：
   ...
   - 删除原有 HBase 集群里的所有数据。通过 ZooKeeper 客户端删除 `zookeeper.znode.parent` 配置的 znode（默认 /hbase）。
   - 将原有 HBase 集群数据迁移到新的 SFS HBase 集群。详细迁移步骤请参考[迁移数据](/sfs/1.3.0/sfs_configuration/sfs_configuration_31#hbase)章节。
4. 参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hbase)章节内容确认 SFS 可用。

## 升级 SFS Hadoop 客户端

1. 将最新的 SFS-hadoop 安装文件更新至 `${HADOOP_HOME}/share/hadoop/common/lib/` 目录中。
2. 重启 Hbase 服务使配置生效，若启用 HA，滚动重启即可。
3. 参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hbase)章节内容确认 SFS 可用。
...
1. 参考 [HDFS](/sfs/1.3.0/sfs_configuration/sfs_configuration_28) 章节为 HDFS 安装 SFS Hadoop 客户端和 Kubeconfig 并修改服务配置。
...
3. 参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hive)章节内容确认 SFS 可用。
...
4. 参考[测试验证](/sfs/1.3.0/sfs_configuration/sfs_configuration_22#hive)章节内容确认 SFS 可用。
...
![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_12.png)
...
1. 获取数据库当前数据位置：
   ...
   ![](https://cdn.smartx.com/internal-docs/assets/32e8842b/sfs_configuration_13.png)
2. 迁移数据库：
